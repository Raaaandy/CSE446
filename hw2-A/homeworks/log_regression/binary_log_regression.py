...
class BinaryLogReg:
    ...
    def __init__(self, _lambda: float = 1e-3):
        ...
        self.weight: np.ndarray = np.zeros((784, 1))  # type: ignore
        self.bias: float = 0.0
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        theta = y * (self.bias + X @ self.weight)
        mu = 1 / (1 + np.exp(-theta))
        return mu
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        n = len(y)
        lambda_part = self._lambda * np.sum(np.square(self.weight))
        loss = np.sum(np.log(1/self.mu(X, y))) / n
        return loss + lambda_part
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        for i in np.arange(n):
            gradient[i] = (1 - mu_V[i]) * y[i] * X[i]
        lambda_grad = + 2 * self._lambda * self.weight
        gradient = (-1 / n) * np.sum(gradient, axis=0)
        return gradient + lambda_grad
    ...
    def misclassification_error(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        diff_V = y - predictions
        return np.count_nonzero(diff_V) / len(diff_V)
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for start_idx in range(num_batches):
                end_idx = min((start_idx + 1) * batch_size, len(X_train))
                X_batch = X_train_shuffled[start_idx * batch_size:end_idx]
                y_batch = y_train_shuffled[start_idx * batch_size:end_idx]
...
if __name__ == "__main__":
   ...
   # GD batchsize = datasize
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.title("Loss / J(w, b) v.s Epochs by GD, learning rate = 1")
    plt.legend()
    ...
    plt.title("Error v.s Epochs by GD, learning rate = 1")
    plt.legend()

    # batch_size = 1, learning rate = 0.01
    history1 = model.train(x_train, y_train, x_test, y_test,
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.title("Loss / J(w, b) v.s Epochs by SGD, learning rate = 0.0001")
    plt.legend()
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Misclassification Error")
    plt.title("Error v.s Epochs by SGD, learning rate = 0.0001")
    plt.legend()

    history2 = model.train(x_train, y_train, x_test, y_test,
                           learning_rate=0.005,
                           batch_size=100)
    # Plot losses
    plt.figure(5)
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.title("Loss / J(w, b) v.s Epochs by mini Batch, learning rate = 0.005")
    plt.legend()
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Misclassification Error")
    plt.title("Error v.s Epochs by mini Batch, learning rate = 0.005")
    plt.legend()
    plt.show()
